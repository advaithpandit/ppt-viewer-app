...
async function fetchAndInitialize() {
    ...
    if (!activePresentationId || !allPresentations.some(p => p.id === activePresentationId)) {
        if (allPresentations.length > 0) {
            displayViewer(allPresentations[0]);
        } else {
...
function displayPresentationsInDropdown() {
    pptList.innerHTML = '';
    if (allPresentations.length === 0) {
        pptList.innerHTML = '<li style="padding: 12px 16px; color: var(--text-secondary);">No files uploaded yet.</li>';
        return;
    }
    allPresentations.forEach((pres) => {
        ...
        if (pres.id === activePresentationId) {
            li.classList.add('active-item');
        }
        // CHANGED: Use SVG icons
        li.innerHTML = `
            <div class="ppt-info">
...
async function performDelete() {
    ...
    if (dbError) {
        alert("File was deleted, but failed to delete database record: " + dbError.message);
    }
...
async function handleUpload(event) {
    ...
    if (!uploaderName || !file) {
        uploadStatus.textContent = 'Please fill in name and choose a file.';
        return;
    }
...
// Floating Menu Button Listener
menuButton.addEventListener('click', (e) => {
    e.stopPropagation();
    dropdownMenu.classList.toggle('show');
});

// Listener for the Upload link in the dropdown
...
// Event Delegation for the list of presentations
pptList.addEventListener('click', (event) => {
    const button = event.target.closest('.action-btn');
    if (button) {
        // NEW: Add popping animation
        button.classList.add('active-pop');
        setTimeout(() => button.classList.remove('active-pop'), 150);
    }
